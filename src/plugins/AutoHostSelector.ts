...
export class AutoHostSelector extends LobbyPlugin {
  ...
  constructor(lobby: Lobby, option: Partial<AutoHostSelectorOption> = {}) {
    ...
    const deny_list = this.LoadDenyListFromFile(this.lobby.option.list_config_path);
    if (Array.isArray(deny_list)) {
      deny_list.map(s => this.lobby.GetOrMakePlayer(s)).forEach(p => DENY_LIST.addPlayer(p));
    }
    this.registerEvents();
  }

  LoadDenyListFromFile(filePath: string): string[] {
    try {
      ...
      this.logger.error(`Failed to load deny list from file: ${filePath}`, error);
      return [];
    }
  }
  ...
  onPrivateCommand(user: Player, command: string, param: string): void {
    ...
    if (!DENY_LIST.includes(player)) {
      const ename = user.escaped_name;
      fs.writeFile(this.lobby.option.list_config_path, `\n${playerName}`,
        err => {
          if (err) {
            this.logger.error('Failed to update deny list file:', err);
          } else {
            DENY_LIST.addPlayer(player);
            this.lobby.SendPrivateMessage(`${gotKicked?`Banned player "${playerName}" from current lobby and`:`Did not find player "${playerName}" in current lobby but`} added to the deny list permanently.`, ename);
          }
        }
      );
    } else {
      this.lobby.SendPrivateMessage(`Player "${playerName}" is already in the deny list. ${gotKicked?'Banned player from current lobby.':'Did not find player in current lobby.'}`, user.escaped_name,);
      }
    }
  }
